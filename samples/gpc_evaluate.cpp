...
const float outlier_threshold = 3.0; // according to the GPC paper
                                     // Shenlong Want, et.al., The global patch collider, CVPR 2016.
...
//const int nTrees = 5;
#include "gpc_const.hpp" // moved and defined here
...
int main( int argc, const char **argv )
{
    ...
    if (from.empty() || to.empty() || gt.empty()) {
        std::cerr << "Input file read error. Check paths." ;
        return 1;
    }
    std::cerr << "@ Images have been loaded.\n";
    ...
    for ( size_t i = 0; i < corr.size(); ++i )
        ...
        // Check that gt flow vector is correct. Only considered is the loc where gt flows is correct
        if (!cvIsNaN(gtDisplacement.x) && !cvIsNaN(gtDisplacement.y) && gtDisplacement.x < 1e9 && gtDisplacement.y < 1e9)
            ...
            double l2error = normL2( b - c );
            if (l2error < outlier_threshold)
                {
                error += l2error;

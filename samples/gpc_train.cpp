...
int main( int argc, const char **argv )
{
  ...
  if ( parser.has( "max-tree-depth" ) )
    params.maxTreeDepth = parser.get< unsigned >( "max-tree-depth" ); // 12
  ...
  fprintf(stderr, "@ numOfTrees   = %d\n", nTrees);
  fprintf(stderr, "@ maxTreeDepth = %d\n", params.maxTreeDepth);
  fprintf(stderr, "@ minNumOfSamp = %d\n", params.minNumberOfSamples);
